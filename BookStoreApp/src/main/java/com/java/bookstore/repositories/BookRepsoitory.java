...
import com.java.bookstore.service.impl.ServiceConstants;
...
public class BookRepsoitory implements BasicCrud<Book> {
	...
	/**
	 * Making a global books list because I am going to mock book database. So as
	 * soon as the spring application boots up this data structure will take care of
	 * books for us.
	 */
	...
	public void init() {
		...
		try {
			...
			for (final Entry<Object, Object> entry : properties.entrySet()) {
				...
				tempBook.setName(arrKey[ServiceConstants.INDEX_ZERO]);
				tempBook.setAuthor(arrKey[ServiceConstants.INDEX_ONE]);
				tempBook.setNumberOfBooks(Integer.parseInt(entry.getValue().toString()));
				tempBook.setIsbn(++isbnCounter + ServiceConstants.PRIMARY_KEY_CONSTANT);
